...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Ventord")
	var updateFlag = flag.Bool("repair", false, "Repair Ventord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Ventord")
	...
	if *versionFlag {
		fmt.Println("Ventord Installer Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2023 Vendicated and Ventord contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Ventord Installer' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest Ventord files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Ventord will likely not work.")
}
